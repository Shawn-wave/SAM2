...
# ================================
VIDEO_PATH = "./data/video/test_person.mp4"  # 입력 비디오 경로
SAM_PATH = "./segment-anything-2"  # SAM2 디렉토리 경로
CHECKPOINT = "sam2_hiera_large.pt"  # 체크포인트 파일명
OUTPUT_BASE_DIR = "./output"  # 출력 기본 디렉토리
# ================================
...
if not os.path.exists(VIDEO_PATH):
    ...
    print("Please update VIDEO_PATH in the code to point to your video file.")
    exit(1)

# 비디오 이름 추출 (확장자 제거)
video_name = os.path.basename(VIDEO_PATH).split(".")[0]

# 출력 디렉토리 설정 - data와 같은 레벨의 output 폴더 사용
...
for out_frame_idx in range(0, len(frame_names)):
    for out_obj_id, out_mask in video_segments[out_frame_idx].items():
        ...
        np.save(os.path.join(dir_name_this_obj, '{:05d}.npy'.format(out_frame_idx)), mask)
